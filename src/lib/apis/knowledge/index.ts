...
export const getKnowledgeBases = async (token: string = '', page: number | null = null) => {
	...
	const res = await fetch(
		`${WEBUI_API_BASE_URL}/knowledge/${queryString ? `?${queryString}` : ''}`,
		{
			method: 'GET',
			headers: {
				Accept: 'application/json',
...
export const exportKnowledgeById = async (token: string, id: string, name: string) => {
	...
	a.href = url;
	a.download = `${name.replace(/\s+/g, '_').substring(0, 50)}.zip`;
