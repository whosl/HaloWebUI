...
export const isSafeMarkdownUrl = (
	value: unknown,
	{ allowHash = true, allowRelative = true, allowDataImage = false }: SafeUrlOptions = {}
): value is string => {
...
const normalizeWebuiBaseUrl = (webuiBaseUrl: string) => String(webuiBaseUrl ?? '').replace(/\/$/, '');
...
export const resolveLocalFileIframeSrcFromHtml = (
	...
	const srcMatch = /<iframe\b[^>]*\ssrc\s*=\s*(?:"([^"]*)"|'([^']*)'|([^\s"'=<>`]+))/i.exec(
		html
	);
